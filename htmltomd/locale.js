...
var supportedLocales_ = null;
var localeStats_ = null;
var loadedLocales_ = {};
var defaultLocale_ = 'en_GB';
var currentLocale_ = defaultLocale_;
...
function localeStats() {
    if (!localeStats_)
        localeStats_ = require('./locales/index.js').stats;
    return localeStats_;
}
function supportedLocales() {
    ...
    var output = [];
    for (var n in supportedLocales_) {
        if (!supportedLocales_.hasOwnProperty(n))
            continue;
        output.push(n);
    }
    return output;
...
function supportedLocalesToLanguages(options) {
    if (options === void 0) { options = null; }
    if (!options)
        options = {};
    var stats = localeStats();
    var locales = supportedLocales();
    var output = {};
    for (var i = 0; i < locales.length; i++) {
        var locale = locales[i];
        output[locale] = countryDisplayName(locale);
        var stat = stats[locale];
...
exports.supportedLocalesToLanguages = supportedLocalesToLanguages;
function closestSupportedLocale(canonicalName, defaultToEnglish, locales) {
    if (defaultToEnglish === void 0) { defaultToEnglish = true; }
    if (locales === void 0) { locales = null; }
    locales = locales === null ? supportedLocales() : locales;
    if (locales.indexOf(canonicalName) >= 0)
        return canonicalName;
    var requiredLanguage = languageCodeOnly(canonicalName).toLowerCase();
    for (var i = 0; i < locales.length; i++) {
        var locale = locales[i];
        var language = locale.split('_')[0];
        if (requiredLanguage == language)
            return locale;
    }
    return defaultToEnglish ? 'en_GB' : null;
}
exports.closestSupportedLocale = closestSupportedLocale;
function countryName(countryCode) {
    var r = codeToCountry_[countryCode] ? codeToCountry_[countryCode] : null;
    if (!r)
        return '';
...
function languageName(languageCode, defaultToEnglish) {
    if (defaultToEnglish === void 0) { defaultToEnglish = true; }
    if (codeToLanguage_[languageCode])
        return codeToLanguage_[languageCode];
    if (defaultToEnglish)
        return languageNameInEnglish(languageCode);
    return '';
}
...
exports.languageCodeOnly = languageCodeOnly;
function countryCodeOnly(canonicalName) {
    if (canonicalName.length <= 2)
        return '';
    return canonicalName.substr(3);
}
exports.countryCodeOnly = countryCodeOnly;
function countryDisplayName(canonicalName) {
    var languageCode = languageCodeOnly(canonicalName);
    var countryCode = countryCodeOnly(canonicalName);
    var output = languageName(languageCode);
    var extraString;
    if (countryCode) {
    ...
    if (languageCode == 'zh' && (countryCode == '' || countryCode == 'TW'))
        extraString = '繁體'; // "Traditional" in "Traditional Chinese"
    if (extraString) {
        output += " (" + extraString + ")";
    }
    else if (countryCode) {
...
function localeStrings(canonicalName) {
    var locale = closestSupportedLocale(canonicalName);
    if (loadedLocales_[locale])
...
function setLocale(canonicalName) {
    if (currentLocale_ == canonicalName)
        return;
...
function localesFromLanguageCode(languageCode, locales) {
    return locales.filter(function (l) {
        return languageCodeOnly(l) === languageCode;
    });
}
exports.localesFromLanguageCode = localesFromLanguageCode;
function _(s) {
    var args = [];
    for (var _i = 1; _i < arguments.length; _i++) {
        args[_i - 1] = arguments[_i];
    }
    var strings = localeStrings(currentLocale_);
    var result = strings[s];
    if (result === '' || result === undefined)
        result = s;
    try {
        return sprintf.apply(void 0, [result].concat(args));
    }
    catch (error) {
...
function _n(singular, plural, n) {
    ...
    if (n > 1)
        return _.apply(void 0, [plural].concat(args));
    return _.apply(void 0, [singular].concat(args));
}
exports._n = _n;
